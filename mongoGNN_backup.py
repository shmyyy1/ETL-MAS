...
def compute_optimal_control(agent_type, current_state, goal_position):
    ...
    Returns control action based on agent dynamics.
    """
    config = AGENT_TYPES[agent_type]
    ...
    if agent_type == "single_integrator":
        ...
        # Acceleration control towards goal
        position = current_state[:2]
        velocity = current_state[2:4]
        direction = goal_position - position
        distance = np.linalg.norm(direction)

        if distance > 0.1:
            # Proportional control on velocity
            desired_velocity = direction / distance * config["max_velocity"] * 0.8
            acceleration_cmd = (desired_velocity - velocity) * 2.0  # P gain
            acceleration_cmd = np.clip(acceleration_cmd, -2.0, 2.0)  # Limit acceleration
        else:
            acceleration_cmd = -velocity * 1.0  # Brake to stop

        return acceleration_cmd

    elif agent_type == "quadrotor":
        # Thrust and attitude commands towards goal
        ...
        if distance > 0.1:
            ...
            desired_vel_xy = direction[:2] / distance * config["max_velocity"] * 0.8

            # Thrust to counteract gravity and provide vertical control
            thrust = 9.81 + (goal_position[2] - position[2]) * 0.5  # Gravity compensation + altitude control
            thrust = np.clip(thrust, 5.0, 20.0)

            # Attitude commands (simplified - roll/pitch for horizontal movement)
            roll_cmd = (desired_vel_xy[1] - velocity[1]) * 0.1  # Simplified dynamics
            pitch_cmd = (desired_vel_xy[0] - velocity[0]) * 0.1
            yaw_cmd = 0.0  # No yaw control for now

            return np.array([thrust, roll_cmd, pitch_cmd, yaw_cmd])
        else:
            # Hover at goal
            thrust = 9.81
            return np.array([thrust, 0.0, 0.0, 0.0])
...
def simulate_dynamics(agent_type, state, action, dt):
    ...
    if agent_type == "single_integrator":
        ...
        # action = [thrust, roll_cmd, pitch_cmd, yaw_cmd]
        # state = [pos_x,y,z, vel_x,y,z, roll,pitch,yaw, ang_vel_x,y,z]

        thrust, roll_cmd, pitch_cmd, yaw_cmd = action

        # Simplified attitude dynamics
        new_state[6:9] += np.array([roll_cmd, pitch_cmd, yaw_cmd]) * dt  # Attitudes
        new_state[9:12] = np.array([roll_cmd, pitch_cmd, yaw_cmd]) * 0.1  # Angular velocities

        # Thrust to acceleration (simplified)
        acc_x = thrust * np.sin(new_state[7])  # pitch affects forward accel
        acc_y = -thrust * np.sin(new_state[6])  # roll affects lateral accel
        acc_z = thrust - 9.81  # gravity

        new_state[3:6] += np.array([acc_x, acc_y, acc_z]) * dt  # Integrate velocity
        new_state[:3] += new_state[3:6] * dt  # Integrate position
...
def select_environment():
    ...
    # For demo purposes, auto-select single_integrator
    # In real usage, uncomment the input lines below
    print("\n[DEMO MODE] Auto-selecting: SINGLE_INTEGRATOR")
...
def run_interactive_training():
    ...
    combined_history["safety_losses"] = safety_history["safety_losses"][:len(gnn_history["epochs"])]
    ...
    print(f"\n🎬 Running simulation demonstration...")
    optimal_traj, safe_traj, obstacles = demonstrate_safety_controller(selected_agent, num_steps=12)

    print(f"\n🎥 Creating simulation video...")
    create_simulation_video(selected_agent, optimal_traj, safe_traj, obstacles,
                          filename=f"simulation_{selected_agent}.mp4")
    ...
    print(f"  - Simulation Video: simulation_{selected_agent}.mp4")
    print(f"  - Training Histories: *_training_history.json")

def demonstrate_safety_controller(agent_type="single_integrator", num_steps=10):
    """
    Demonstrate the safety controller in action.
    Shows optimal vs safe trajectories.
    """
    print(f"\n🛡️ Demonstrating Safety Controller for {agent_type}")
    print("=" * 50)
    ...
    start_pos = np.array([1.0, 1.0]) if agent_type != "quadrotor" else np.array([1.0, 1.0, 2.0])
    goal_pos = np.array([8.0, 8.0]) if agent_type != "quadrotor" else np.array([8.0, 8.0, 5.0])
    ...
    if agent_type == "single_integrator":
        ...
        state = np.concatenate([start_pos, np.zeros(9)])
    ...
    for step in range(num_steps):
        ...
        # Build graph for GNN (simplified single agent)
        positions = state[:2] if agent_type != "quadrotor" else state[:2]  # 2D projection
        ...
        if agent_type == "quadrotor":
            features = torch.cat([
                ...
                torch.norm(torch.tensor(state[3:6], dtype=torch.float)).unsqueeze(0).unsqueeze(0),
                torch.tensor([[0.0]], dtype=torch.float)
            ], dim=1)
        else:
            features = torch.cat([
                torch.tensor(state, dtype=torch.float).unsqueeze(0),
                torch.norm(torch.tensor(state[2:4], dtype=torch.float)).unsqueeze(0).unsqueeze(0),
                torch.tensor([[0.0]], dtype=torch.float)
            ], dim=1)

        # Get safety prediction
        with torch.no_grad():
            safety_score = gnn_model(features, edge_index).squeeze()

        # Get safe action
        safe_action = safety_controller(
            torch.tensor(state, dtype=torch.float),
            torch.tensor(optimal_action, dtype=torch.float),
            safety_score
        )

        # Simulate optimal trajectory
        optimal_state = simulate_dynamics(agent_type, state, optimal_action, dt)
        optimal_trajectory.append(optimal_state)

        # Simulate safe trajectory
        safe_state = simulate_dynamics(agent_type, state, safe_action.detach().numpy(), dt)
        safe_trajectory.append(safe_state)

        # Update state for next iteration (using safe action)
        state = safe_state

        print(f"Step {step+1}: Safety={safety_score:.3f}, |Optimal-Safe|={np.linalg.norm(optimal_action - safe_action.detach().numpy()):.3f}")

    # Compute final safety scores
    optimal_positions = np.array([s[:2] for s in optimal_trajectory])
    safe_positions = np.array([s[:2] for s in safe_trajectory])

    optimal_safety, _ = compute_collision_risk(optimal_positions, np.zeros((len(optimal_positions), 2)), obstacles, config["safety_threshold"])
    safe_safety, _ = compute_collision_risk(safe_positions, np.zeros((len(safe_positions), 2)), obstacles, config["safety_threshold"])

    print(f"\nTrajectory Safety Scores:")
    print(f"Optimal trajectory: min={optimal_safety.min():.3f}, avg={optimal_safety.mean():.3f}")
    print(f"Safe trajectory: min={safe_safety.min():.3f}, avg={safe_safety.mean():.3f}")

    return optimal_trajectory, safe_trajectory, obstacles
